...
context_data <- wvs %>% group_by(B_COUNTRY) %>% 
  summarise(gdp = mean(GDPpercap1, na.rm = TRUE),
         life_exp = mean(lifeexpect, na.rm = TRUE),
         gini = mean(giniWB, na.rm = TRUE)) %>% 
  rename(idenpa=B_COUNTRY)
context_data$idenpa <- as.numeric(context_data$idenpa)


proc_data <- proc_data %>% group_by(idenpa) %>% 
  summarise(promedio = mean(conf_inst, na.rm = TRUE))

data1 <- merge(proc_data, context_data, by="idenpa")
data2 <- right_join(proc_data, context_data, by="idenpa")
data3 <- full_join(proc_data, context_data, by="idenpa")
data <- left_join(proc_data, context_data, by="idenpa")
